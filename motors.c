...
void *tcrtLeftSensor(void *arg){
    while(1){
        int tcrtValue = gpioRead(TCRT_LEFT);
...
void *tcrtRightSensor(void *arg){
    while(1){
        int tcrtValue = gpioRead(TCRT_RIGHT);
	 printf("Right sensor value: %d\n", tcrtValue);

        if (tcrtValue == 1) {
            printf("Right sensor is on the Black Line!\n");
            // Adjust direction to the left
        } else {
            printf("Right sensor is not on the Black Line\n");
        }
...
void testSensors(int seconds, pthread_t thread1, pthread_t thread2) {
    ...
    // Cancel the sensor threads
    pthread_cancel(thread1);
    pthread_cancel(thread2);

    // Wait for the sensor threads to finish
...
int main() {
  ...
  gpioSetMode(TCRT_LEFT, PI_INPUT);
  gpioSetMode(TCRT_RIGHT, PI_INPUT);
  ...
  pthread_t tcrtLeftThread, tcrtRightThread;

  pthread_create(&tcrtLeftThread, NULL, tcrtLeftSensor, NULL);
  pthread_create(&tcrtRightThread, NULL, tcrtRightSensor, NULL);

  testSensors(7, tcrtLeftThread, tcrtRightThread);
